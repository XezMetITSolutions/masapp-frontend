...
export default function EditRestaurantForm() {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      language: restaurant.settings?.language || ['tr'],
      currency: restaurant.settings?.currency || 'TRY',
      taxRate: restaurant.settings?.taxRate ?? 18,
      serviceChargeRate: restaurant.settings?.serviceChargeRate ?? 0,
      allowTips: restaurant.settings?.allowTips ?? true,
      allowOnlinePayment: restaurant.settings?.allowOnlinePayment ?? true,
    ...
    try {
      const updates: Partial<Restaurant> = {
        ...
        settings: {
          ...
          workingHours: restaurant.settings?.workingHours || [],
        },
        subscription: {
          ...(restaurant.subscription || {}),
          plan: formData.plan,
          status: restaurant.subscription?.status || 'active',
          startDate: restaurant.subscription?.startDate || new Date(),
          endDate: restaurant.subscription?.endDate || new Date(new Date().setFullYear(new Date().getFullYear() + 1)),
        },
      };
  ...
  if (!restaurant) {
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Restoran bulunamadı</h2>
          <p className="text-gray-600 mb-6">Düzenlemek istediğiniz restoran mevcut değil veya yükleniyor...</p>
          <Link href="/admin/restaurants" className="px-4 py-2 bg-blue-600 text-white rounded-lg">Geri Dön</Link>
        </div>
      </div>
    );
      ...
      <div className="ml-0 md:ml-0">
        <header className="bg-white shadow-sm border-b">
          <div className="px-8 py-4 flex items-center">
            <Link href="/admin/restaurants" className="mr-4 text-gray-600 hover:text-gray-800">
              <FaArrowLeft />
            </Link>
            <h2 className="text-2xl font-semibold text-gray-800">Restoranı Düzenle</h2>
          </div>
        </header>
